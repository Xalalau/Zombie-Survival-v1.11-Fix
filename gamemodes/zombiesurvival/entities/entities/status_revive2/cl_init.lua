	...
	if owner:IsValid() then
		...
		if rag and rag:IsValid() then
			...
			if endtime - 1 <= ct then
				rag:SetModel("models/player/classic.mdl")
				...
				rag:GetPhysicsObject():ComputeShadowControl({secondstoarrive = 0.05, pos = owner:GetPos() + Vector(0,0,16), angle = rag:GetPhysicsObject():GetAngle(), maxangular = 2000, maxangulardamp = 10000, maxspeed = 5000, maxspeeddamp = 1000, dampfactor = 0.85, teleportdistance = 200, deltatime = FrameTime()})
			end
		end
	end
