...
local vecNothing = Vector(0.0001, 0.0001, 0.0001)
...
local function Dismember_BuildBonePositions(eRagdoll, iNumBones, iNumPhysBones)
	if eRagdoll:IsValid() then
		local emitter = eRagdoll.Emitter
		local iDismemberment = eRagdoll.Dismemberment or 0
		for index, iDismemberPart in pairs(Dismembers) do
			if iDismemberPart & iDismemberment == iDismemberPart then
				for _, sZeroBone in pairs(DismemberBones[index].ToZero) do
					...
					if iBone then
						local mMatrix = eRagdoll:GetBoneMatrix(iBone)
						mMatrix:Scale(vecNothing)
						eRagdoll:SetBoneMatrix(iBone, mMatrix)
					end
				end
			end
		end
...
function EFFECT:Think()
	...
	if not self.SetDoll and eEnt:IsPlayer() then
		local eRag = eEnt:GetRagdollEntity()
		if eRag and eRag:IsValid() then
			self.SetDoll = true
			self.eRagdoll = eRag
			eRag.Emitter = ParticleEmitter(eRag:GetPos())
			eRag.Emitter:SetNearClip(20, 30)
			eRag.Dismemberment = (eRag.Dismemberment or 0) | self.iScale
			eRag.BuildBonePositions = Dismember_BuildBonePositions
						...
						if 0 < delta then
							...
							if vBonePos and aBoneAng then
								...
								for i=1, math.random(0, 2) do
									local particle = emitter:Add("noxctf/sprite_bloodspray"..math.random(1,8), vBonePos)
