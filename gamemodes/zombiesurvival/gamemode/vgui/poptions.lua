...
function MakepOptions()
	...
	slider:SetText("Detail Props")

	local check = vgui.Create("DCheckBoxLabel", Window)
	check:SetPos(32, 190)
	check:SetSize(wide, 32)
	check:SetText("Draw flashlights and shadows")
	check:SetConVar("r_shadows")

	local check = vgui.Create("DCheckBoxLabel", Window)
	check:SetPos(32, 220)
	check:SetSize(wide, 32)
	check:SetText("Old crosshair style")
	check:SetConVar("zs_oldcrosshair")

	local check = vgui.Create("DCheckBoxLabel", Window)
	check:SetPos(32, 250)
	check:SetSize(wide, 32)
	check:SetText("No baby sound")
	check:SetConVar("zs_nobaby")

	local check = vgui.Create("DCheckBoxLabel", Window)
	check:SetPos(32, 280)
	check:SetSize(wide, 32)
	check:SetText("Play last human and other music")
	...
	local check = vgui.Create("DCheckBoxLabel", Window)
	check:SetPos(wide + 32, 200)
	check:SetSize(wide, 32)
	check:SetText("Enable Human Auras")
	check:SetConVar("_zs_enableauras")
	check.Button.ConVarChanged = CheckChanged

	local slider = vgui.Create("DNumSlider", Window)
	slider:SetPos(wide + 32, 230)
	slider:SetSize(200, 48)
