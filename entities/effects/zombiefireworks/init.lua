function EFFECT:Init(data)
	...
	EmitSound("ambient/explosions/exp3.wav", pos, 0, CHAN_AUTO, 1, 80, 0, math.random(100, 110))
...
function EFFECT:Think()
	...
	if RealTime() >= self.DieTime then
		EmitSound("ambient/explosions/explode_"..math.random(2,5)..".wav", pos, 0, CHAN_AUTO, 1, 90, 0, math.random(100, 110))
		local emitter = ParticleEmitter(pos)
			for i=1, math.random(200, 230) do			
				particle = emitter:Add("sprites/light_glow02_add", pos + VectorRand():GetNormalized() * 100)
				particle:SetVelocity(VectorRand():GetNormalized() * 700)
...
function EFFECT:Render()
	...
	local emitter = ParticleEmitter(pos)
		for i=1, 4 do
			...
			particle:SetVelocity(self.Entity:GetVelocity() * -0.4 + VectorRand():GetNormalized() * 20)
