...
local function CollideCallback(particle, hitpos, hitnormal)
	...
	local emitter = ParticleEmitter(pos)
		for i=1, math.random(-4, 4) do
			...
			particle:SetVelocity(VectorRand():Normalize() * math.random(75, 150) + nhitnormal)
...
function EFFECT:Init(data)
	...
	local emitter = ParticleEmitter(Pos)
		for i=1, data:GetMagnitude() do
			...
			//particle:SetVelocity((VectorRand():Normalize() * (math.random(30, 70) / 180) + VectorRand():Normalize():Angle()) * 80)
